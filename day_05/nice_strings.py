...
def is_super_nice_string(string):
    ...
    for index, letter, previous_letter, previous_previous_letter in zip(range(len(string)), string, ' ' + string, '  ' + string):
        ...
        three_letter_palindrome = True if previous_letter + letter in letter_pairs and functools.reduce(lambda palindrome_exists, old_index: palindrome_exists or index - old_index > 1, letter_pairs[previous_letter + letter], False) else three_letter_palindrome
